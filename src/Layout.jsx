...
export default function Layout({ children, currentPageName }) {
            ...
            return (
                <details className="group" open>
                    <summary className={`relative flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-all font-black text-xs tracking-[0.15em] uppercase font-mono select-none ${isActive ? activeCls : inactiveCls}`}>
                        ...
                        }) && <span className="absolute left-0 inset-y-0 w-0.5 rounded-full bg-[#10b981]" />}
    ...
    return (
        <div className="flex h-screen bg-stone-50 dark:bg-[#111413] flex-col md:flex-row font-mono">
            {/* Desktop Sidebar */}
            <div className="hidden md:flex md:w-56 bg-white dark:bg-[#0d0f0e] border-r border-black/10 dark:border-white/5 flex-col">
                {/* Logo */}
            ...
            <div className="flex-1 flex flex-col overflow-auto">
                {/* Mobile Header */}
                <div className="md:hidden bg-white dark:bg-[#0d0f0e] border-b border-black/10 dark:border-white/5 px-4 py-3 flex items-center justify-between">
                    <Link to="/">
                ...
                {/* Mobile Menu */}
                {mobileMenuOpen && (
                    <div className="md:hidden bg-white dark:bg-[#0d0f0e] border-b border-black/10 dark:border-white/5 px-3 py-3 space-y-0.5 max-h-[calc(100vh-52px)] overflow-y-auto">
                        {navItems.map((item) => (
