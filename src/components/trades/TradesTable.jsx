                ...
                <Table style={{ minWidth: '2000px' }}>
                    <TableHeader style={{ position: 'sticky', top: 0, zIndex: 20, backgroundColor: 'white' }}>
                        <TableRow className="bg-slate-50 border-b border-slate-200">
                            ...
                            <TableHead onClick={() => handleSort('open_value')} className="font-semibold text-slate-700 whitespace-nowrap text-xs py-1 px-2 cursor-pointer hover:bg-slate-100">
                                Open Value<SortIcon field="open_value" />
                            </TableHead>
                            ...
                            <TableHead onClick={() => handleSort('current_value')} className="font-semibold text-slate-700 whitespace-nowrap text-xs py-1 px-2 cursor-pointer hover:bg-slate-100">
                            ...
                            <TableHead onClick={() => handleSort('profit')} className="font-semibold text-slate-700 whitespace-nowrap text-xs py-1 px-2 cursor-pointer hover:bg-slate-100">
                    ...
                    <TableBody>
                        {trades.length === 0 ? (
                            ...
                            visibleTrades.map((trade, index) => {
                                const calculatedProfit = (trade.open_value || 0) + (trade.current_value || 0) + (trade.collateral_gain || 0);
                                return (
                                <TableRow key={trade.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <TableCell className="text-slate-600 text-xs py-1 px-2 text-right">{formatCurrency(trade.open_value)}</TableCell>
                                    <TableCell className="text-slate-600 text-xs py-1 px-2 text-right">{formatPercent(trade.potential_yield)}</TableCell>
                                    <TableCell className="text-slate-600 text-xs py-1 px-2 text-right">{formatCurrency(trade.current_value)}</TableCell>
